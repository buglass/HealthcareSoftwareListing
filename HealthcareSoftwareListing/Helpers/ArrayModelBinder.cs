...
namespace HealthcareSoftwareListing.Helpers
{
	public class ArrayModelBinder : IModelBinder
	{
		public Task BindModelAsync(ModelBindingContext bindingContext)
		{
			...
			// Now get a converter which will (in this case) convert string types to guids
			var converter = TypeDescriptor.GetConverter(elementType);

			// Split the string delimited value and convert each string in turn
			var values = value.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries)
				.Select(valueFromString => converter.ConvertFromString(valueFromString.Trim()))
				.ToArray();
